...
export class Vehicle {
    ...
    @Column({ type: 'enum', enum: ['Gasolina', 'Diesel', 'Eléctrico', 'Híbrido', 'GLP', 'GNC'] })
    ...
    @Column({ name: 'seguro_numero_poliza' })
    seguroNumeroPoliza!: string;

    @Column({ type: 'date', name: 'seguro_fecha_vencimiento' })
    seguroFechaVencimiento!: Date;

    @Column({ type: 'enum', enum: ['Terceros', 'Terceros ampliado', 'Todo riesgo', 'Todo riesgo con franquicia'], name: 'seguro_cobertura' })
    ...
    @Column({ type: 'date', name: 'itv_fecha_caducidad' })
