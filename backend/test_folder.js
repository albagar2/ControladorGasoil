...
async function checkFolder() {
    ...
    try {
        ...
        require('fs').writeFileSync('folder_check_result.txt', JSON.stringify({ status: 'SUCCESS', data: res.data }));
    } catch (e) {
        console.log("ERROR! Could not access folder:");
        console.log(e.message);
        require('fs').writeFileSync('folder_check_result.txt', JSON.stringify({ status: 'ERROR', error: e.message }));
    }
}
